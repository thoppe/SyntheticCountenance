...
class Pipeline:
    ...
    def __init__(
        ...
        if not recursive:
            ...
            F_IN = glob.glob(
                os.path.join(load_dest, '**', old_glob), recursive=True)
            F_OUT = glob.glob(
                os.path.join(save_dest, '**', new_glob), recursive=True)
    ...
    def get_output_file(self, f):
        ...
        if self.recursive:
            local_save_dest = os.path.join(
                self.save_dest,
                os.path.split(
                    os.path.normpath(f.replace(self.load_dest, '')))[0],
            )

            if not self.load_only and not os.path.exists(local_save_dest):
                os.makedirs(local_save_dest)
                
        else:
            local_save_dest = self.save_dest


        return os.path.join(local_save_dest, base)
