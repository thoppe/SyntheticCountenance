...
class AestheticsNet:
    ...
    def channels_first(self, myinput=None):
        ...
        if len(channel_dim) != 1:
            raise ValueError(
                "Error during memnet preprocessing. Couldn't identify the channel dimension"
            )
        else:
            channel_dim = channel_dim[0]
            myinput = tf.transpose(
                myinput,
                [0] + [channel_dim] + [i for i in range(1, 4) if i != channel_dim],
            )
        return myinput
    ...
    def aestheticsnet_preprocess(self, myinput=None):
        mu = self.get_mu(
            os.path.join(
                ...
                "mean_AADB_regression_warp256_lore.npy",
            )
        )
        myinput_BGR = tf.reverse(myinput, [3])
        myinput_227 = tf.image.resize_images(
            myinput_BGR, tf.constant([227, 227])
        )  # tensorflow resizing does not work in the exact same way as in the original code
    ...
    def aestheticsnet_fn(self, myinput=None, attribute=None):
        ...
        pool1 = tf.layers.max_pooling2d(
            inputs=conv1, pool_size=3, strides=2, data_format="channels_first"
        )

        # Norm Layer #1
        norm1 = tf.transpose(
            tf.nn.local_response_normalization(
                tf.transpose(pool1, [0, 2, 3, 1]),
                depth_radius=2,
                alpha=0.00002,
                beta=0.75,
                bias=1,
        ...
        conv2_group1 = tf.layers.conv2d(
            inputs=norm1_padd_group1,
            data_format="channels_first",
            filters=128,
            kernel_size=[5, 5],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv2.weight"].numpy()[0:128, :, :, :], [2, 3, 1, 0]
                )
            ),
        ...
        conv2_group2 = tf.layers.conv2d(
            inputs=norm1_padd_group2,
            data_format="channels_first",
            filters=128,
            kernel_size=[5, 5],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv2.weight"].numpy()[128:256, :, :, :], [2, 3, 1, 0]
                )
            ),
            bias_initializer=tf.constant_initializer(
        ...
        pool2 = tf.layers.max_pooling2d(
            inputs=conv2, pool_size=3, strides=2, data_format="channels_first"
        )

        # Norm Layer #2
        norm2 = tf.transpose(
            tf.nn.local_response_normalization(
                tf.transpose(pool2, [0, 2, 3, 1]),
                depth_radius=2,
                alpha=0.00002,
                beta=0.75,
                bias=1,
        ...
        conv4_group1 = tf.layers.conv2d(
            inputs=conv3_padded_group1,
            data_format="channels_first",
            filters=192,
            kernel_size=[3, 3],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv4.weight"].numpy()[0:192, :, :, :], [2, 3, 1, 0]
                )
            ),
            bias_initializer=tf.constant_initializer(
        ...
        conv4_group2 = tf.layers.conv2d(
            inputs=conv3_padded_group2,
            data_format="channels_first",
            filters=192,
            kernel_size=[3, 3],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv4.weight"].numpy()[192:384, :, :, :], [2, 3, 1, 0]
                )
            ),
            bias_initializer=tf.constant_initializer(
        ...
        conv5_group1 = tf.layers.conv2d(
            inputs=conv4_padded_group1,
            data_format="channels_first",
            filters=128,
            kernel_size=[3, 3],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv5.weight"].numpy()[0:128, :, :, :], [2, 3, 1, 0]
                )
            ),
            bias_initializer=tf.constant_initializer(
        ...
        conv5_group2 = tf.layers.conv2d(
            inputs=conv4_padded_group2,
            data_format="channels_first",
            filters=128,
            kernel_size=[3, 3],
            strides=[1, 1],
            ...
            kernel_initializer=tf.constant_initializer(
                np.transpose(
                    state_dict["conv5.weight"].numpy()[128:256, :, :, :], [2, 3, 1, 0]
                )
            ),
            bias_initializer=tf.constant_initializer(
        ...
        pool5 = tf.layers.max_pooling2d(
            inputs=conv5, pool_size=3, strides=2, data_format="channels_first"
        )
        ...
        fc8 = tf.layers.dense(
            ...
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_new.bias"].numpy()
            ),
        )

        # BalancingElement
        fc8_BalancingElement = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
                np.transpose(state_dict["fc8_BalancingElement.weight"].numpy(), [1, 0])
            ),
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_BalancingElement.bias"].numpy()
            ),
        )
        BalancingElement = tf.layers.dense(
            inputs=fc8_BalancingElement,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        # ColorHarmony
        fc8_ColorHarmony = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
        ...
        ColorHarmony = tf.layers.dense(
            inputs=fc8_ColorHarmony,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        fc8_Content = tf.layers.dense(
            ...
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_Content.bias"].numpy()
            ),
        )
        Content = tf.layers.dense(
            inputs=fc8_Content,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        fc8_Light = tf.layers.dense(
            ...
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_Light.bias"].numpy()
            ),
        )
        Light = tf.layers.dense(
            inputs=fc8_Light,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        # MotionBlur
        fc8_MotionBlur = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
        ...
        MotionBlur = tf.layers.dense(
            inputs=fc8_MotionBlur,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        fc8_Object = tf.layers.dense(
            ...
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_Object.bias"].numpy()
            ),
        )
        Object = tf.layers.dense(
            inputs=fc8_Object,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        # Repetition
        fc8_Repetition = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
        ...
        Repetition = tf.layers.dense(
            inputs=fc8_Repetition,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        # RuleOfThirds
        fc8_RuleOfThirds = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
        ...
        RuleOfThirds = tf.layers.dense(
            inputs=fc8_RuleOfThirds,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        fc8_Symmetry = tf.layers.dense(
            ...
            bias_initializer=tf.constant_initializer(
                state_dict["fc8_Symmetry.bias"].numpy()
            ),
        )
        Symmetry = tf.layers.dense(
            inputs=fc8_Symmetry,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
        ...
        # VividColor
        fc8_VividColor = tf.layers.dense(
            inputs=fc7,
            units=256,
            activation=tf.nn.relu,
            kernel_initializer=tf.constant_initializer(
        ...
        VividColor = tf.layers.dense(
            inputs=fc8_VividColor,
            units=1,
            activation=None,
            kernel_initializer=tf.constant_initializer(
