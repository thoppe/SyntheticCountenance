...
class MemNet:
    ...
    def tencrop(self, myinput=None):
            ...
            for i in indices:
                for j in indices:
                    ...
                    crops_list = tf.concat(
                        [crops_list, [tf.transpose(temp_img, (2, 0, 1))]], 0
                    )
                    crops_list = tf.concat(
                        [crops_list, [tf.reverse(crops_list[-1], tf.constant([2]))]], 0
                    )
        ...
        _, crops_list, _ = tf.while_loop(
            cond,
            body,
            [myinput, crops_list, i],
            shape_invariants=[
                tf.TensorShape([None, 256, 256, 3]),
                tf.TensorShape([None, 3, 227, 227]),
                i.get_shape(),
            ],
        )
    ...
    def channels_last(self, myinput=None):
        ...
        if len(channel_dim) != 1:
            ...
            myinput = tf.transpose(
                myinput, [i for i in range(4) if i != channel_dim] + [channel_dim]
            )
        return myinput
    ...
    def memnet_preprocess(self, myinput=None):
        ...
        mu = self.get_mu(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "memnet_mean.mat")
        )
        myinput_BGR = tf.reverse(myinput_channels_last, [3])
        myinput_256 = tf.image.resize_images(
            myinput_BGR, tf.constant([256, 256])
    ...
    def memnet_fn(self, myinput=None):
        ...
        state_dict = self.get_weights(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "memnet_state_dict.p"
            )
        )
        ...
        fc8_mean = tf.layers.average_pooling1d(
            inputs=fc8_expand,
            pool_size=[10],
            strides=[10],
            padding="valid",
        ...
        memorability = tf.math.minimum(
            tf.math.maximum(
                (fc8_mean_squeeze - mean_pred) * pred_rescale + mean_pred_add, pred_min
            ),
            pred_max,
        )
