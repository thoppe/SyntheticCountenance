...
class MaximizeDiscriminator:
    def __init__(
        ...
        if f_img is None:
            img0 = np.random.uniform(-1, 1, size=[3, image_dim, image_dim])
        else:
            np_img = np.array(Image.open(f_img))
            img0 = RGB_to_GAN_output(np_img)
            img0 = np.squeeze(img0, axis=0)

        self.img = tf.Variable(img0[None, :, :, :], dtype=tf.float32)

        self.img_org = tf.Variable(img0[None, :, :, :], dtype=tf.float32)

        D_scores, D_labels = discriminator.get_output_for(self.img, is_training=False)
    ...
    def initialize(self):
        self.sess.run(tf.initializers.variables([self.img, self.img_org]))
